...
func RegisterUser(sdc client.StrongDocClient, name, password, email string) (userID, orgID string, success bool, err error) {
	success = false
	err = nil
	...
	srpVerifier, err := srpSession.Verifier([]byte(""), []byte(password))
	if err != nil {
		return
	}
	_, verifierString := srpVerifier.Encode()
