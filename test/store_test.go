...
func TestStore(t *testing.T) {
	...
	// Write to store
	//
	writer, err := store.CreateStore(sdc, &store.StoreInit{
		StoreContent: proto.StoreInit_GENERIC,
		Json:         nil,
		GenericFile:  storeFileName,
	})
	...
	assert.NoError(err)
	reader, err := store.OpenStore(sdc, &store.StoreInit{
		StoreContent: proto.StoreInit_GENERIC,
		Json:         nil,
		GenericFile:  storeFileName,
	})
